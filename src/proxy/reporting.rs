...
use crate::proxy::body::{self, BodyAsyncFrameReporter};
...
const MPSC_BUFFER_SIZE: usize = 1;

pub fn create_buffered(
    event_reporter: Box<dyn EventReporter>,
    direction: Direction,
    prev_bytes: usize,
) -> (Arc<BufferedDataCollector>, BufferedDataReporter) {
    ...
    (Arc::new(collector), data_reporter)
}
...
impl BufferedDataCollector {
    ...
    async fn send(&self, event: BodyEvent) -> Result<()> {
        self.sender
            .send(event)
            .await
            .wrap_err("sending event to BufferedDataReporters failed")
    }
}

impl BodyAsyncFrameReporter for BufferedDataCollector {
    async fn dispatch(self: Arc<Self>, item: Arc<Option<body::BodyStreamItem>>) -> Result<()> {
        match item.as_ref() {
            Some(Ok(frame)) => {
    ...
    async fn test_buffered_basic() {
        let events: Arc<Mutex<Vec<Event>>> = Arc::new(Mutex::new(Vec::new()));
        {
            ...
            let reporter_task = tokio::task::spawn(async move { reporter.run().await });
            {
                ...
                collector.dispatch_bare(Some(dfr(b"ab"))).await.unwrap();
                collector.dispatch_bare(Some(dfr(b"cd"))).await.unwrap();
                collector.dispatch_bare(None).await.unwrap();
            }
