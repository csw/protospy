...
worktree. Work through the steps below in order. Stop and surface blockers
rather than guessing through them.
...
This inner skill must already be running in an isolated Claude Code worktree. It
must not call `EnterWorktree` or run `git worktree add`.
...
The dispatcher should have entered a worktree on the ticket branch. Confirm the
current branch matches the ticket branch before implementation. If it does not,
stop and surface the mismatch rather than switching branches unexpectedly.
