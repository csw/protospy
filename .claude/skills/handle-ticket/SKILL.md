---
name: handle-ticket
description: Run the full protospy Linear-ticket implementation workflow end-to-end from the launcher-created worktree. Launch via `just claude-ticket`/`cs claude-ticket`; do not invoke directly from the main checkout.
...
`## Specific Guidelines` section of `CLAUDE.md` and read every guide whose
...
The launcher already resolved the branch and created the worktree; use the
current branch as-is — do not derive, truncate, create, or check out a branch
yourself unless directions say otherwise. If it does not match `$ticket`, stop
and surface the mismatch. If the user says to start fresh, ignore prior
branches/PRs and continue on the current branch/worktree.
...
the `## Specific Guidelines` trigger list in `CLAUDE.md`: always read
...
`conformance/`, `demo/`), read that subproject's `CLAUDE.md` as well.
