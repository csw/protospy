...
description: >-
  ...
  docs/ui/design-system.md (§4 hard rules, the §3 component decision table,
...
- composition drift and boolean-prop explosions. Primitive selection is yours
  when a §3 row or §4 hard rule requires a stock shadcn/Radix primitive.
...
1. `docs/ui/design-system.md` — **the authority.** Read it in full before
   ...
   rules. Every finding cites a specific rule number, table row, or §.
...
- **Rule 15 — primitive over hand-roll:** when changed code hand-rolls behavior
  that matches a stock shadcn/Radix primitive, flag it even if that exact
  affordance is not listed in §3. The §3 table is illustrative, not exhaustive.
...
**Files reviewed**: N
**Spec**: docs/ui/design-system.md (§4 hard rules, §3 table, §2 token contract)
