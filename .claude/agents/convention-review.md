...
- `vercel-react-best-practices` — React performance optimization: waterfalls,
  bundle size, re-render prevention, data fetching patterns. Apply the review-guide
  / audit mode.
- `vercel-composition-patterns` — React composition architecture: compound
  components, context providers, composition over boolean props, React 19 APIs.
- `shadcn` — shadcn/ui component usage: prefer existing primitives over
  hand-rolled equivalents, correct `cn()` usage, semantic tokens over ad-hoc colour.
- `tailwind-4-docs` — Tailwind v4 semantics: the `@theme inline` mapping, semantic
  design tokens, no-op / undefined token classes, dark-mode correctness, `@apply`
  pitfalls.
