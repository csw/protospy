...
| `stream-paused`               | SSE stream (paused)               | Same messages as `stream-live`; `streamPaused: true` in config initializes `StreamView` in the frozen state. Resume button visible in header; event list frozen at snapshot.          |
...
| `stream-anthropic-transcript` | Anthropic SSE stream (transcript) | Same messages as `stream-anthropic`; `chatStreamTab: "transcript"` in config initializes `ChatStreamView` on the transcript tab. Assembled text output with model metadata and usage. |
...
| `ndjson-text`           | NDJSON body (text view)      | Same NDJSON body as `ndjson` with `responseViewMode: "text"`. Body pane renders the raw newline-delimited text in TextView (Tree/Text/Hex selector shown; Text tab active). Exercises the view-mode selector and text rendering path at all widths, including 1024.         |
