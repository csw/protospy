...
**Data flow:** On mount the app fetches `/info` to discover services, then opens an SSE `EventSource` at `/service/<name>/events`. Each `exchange-report` event is parsed into an `EventMessage` and fed to the store's `applyEvent`, whose pure reducer (`state/reducer.ts`) reassembles request/response **exchanges** keyed by exchange id; the store is bounded by oldest-first FIFO eviction (1024 exchanges / 512 MB of body payload, never evicting the selected one). Components subscribe to store slices and render a virtualized list of exchanges (left) and a detail inspector (right). Bodies are decoded lazily by `body/` (chunk concatenation, gzip/deflate/brotli/zstd decompression, JSON/NDJSON/HTML/XML/SSE detection) once complete; JSON/NDJSON and HTML/XML are each offloaded to a Web Worker (the latter re-indents and syntax-highlights via Prism for the virtualized formatted view).
