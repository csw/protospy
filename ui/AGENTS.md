...
- `src/components/` — app components (flat, kebab-case: `body-split.tsx`, `live-indicator.tsx`, `json-viewer.tsx`, …); v2.4 shell/chrome and list/inspector scaffolds alongside legacy components; `components/anthropic/` for SSE+Anthropic-specific rendering; `components/json-tree/` is the new virtualized JSON tree viewer subpackage (PRO-397 phase 1a — model + flattener + `JsonTreeViewer`, standalone via the dev-only `#json-tree-harness`); vendored shadcn primitives under `components/ui/`
