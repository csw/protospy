# AGENTS.md — protospy UI

Personal, uncommitted UI guidance is harness-specific: Claude Code reads
...
- `src/body/` — `decode.ts` (chunks→bytes→decompress→classify); `json-parse.ts` (pure `parseAndFormat` — used by Worker + tests); `json-parse.worker.ts` (Worker script — JSON.parse + pretty-print); `json-parse-worker.ts` (main-thread client — lazy singleton `parseJson()`); `sse.ts` (parseSSEBlock, parseSSEBody, chunksToText); `sse-stream.ts` (SSEStreamState, feedChunk, applyRetention — incremental SSE parser)
...
- `src/components/` — app components (flat, kebab-case: `body-split.tsx`, `body-pane.tsx`, `live-indicator.tsx`, …); v2.4 shell/chrome and list/inspector scaffolds alongside legacy components; `components/anthropic/` for SSE+Anthropic-specific rendering; `components/json-tree/` is the virtualized JSON tree viewer subpackage (PRO-397/398/399/400 — model + flattener + `JsonTreeViewer`, NDJSON forest display, truncation handling, copy value/path; standalone via the dev-only `#json-tree-harness`) and is the renderer for both `json` and `ndjson` bodies; vendored shadcn primitives under `components/ui/`
...
The `visual-review` subagent automates this check (see the root `AGENTS.md` "Visual
...
[`.claude/rules/ui-testing.md`](../.claude/rules/ui-testing.md). Claude Code may
auto-load that path-scoped rule when you read a file under `ui/browser/`,
`ui/src/__tests__/`, or `ui/src/test/`. Codex agents must read it explicitly
until there is a native Codex equivalent. **Read it before writing or changing a
browser test or a design-token test** — explicitly, since creating a brand-new
test file may not trigger auto-loading in either harness.
