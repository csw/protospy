...
- `src/body/` — `decode.ts` (chunks→bytes→decompress→classify), `sse.ts` (parseSSEBlock, parseSSEBody, chunksToText), `sse-stream.ts` (SSEStreamState, feedChunk, applyRetention — incremental SSE parser), `event-stream.ts` (`readEventStream` → `EventStreamBody` view + `classifyEvent` O2 seam)
