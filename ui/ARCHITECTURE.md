...
- `decodeBody` (`body/decode.ts`) is the async pipeline: concatenate chunks to bytes (`text` chunks UTF-8-encoded, `binary` chunks base64-decoded) → decompress: `gzip`/`deflate` via the browser `DecompressionStream`; `br` (brotli) via `brotli-dec-wasm` (~200 KB WASM, lazy-loaded on first use via a module-level singleton); `zstd` via `@bokuweb/zstd-wasm` (~248 KB WASM, same lazy singleton pattern, `"node"` exports condition used by Vitest so no Node wrapper needed) → `TextDecoder` to text → classify into `kind`: `jsonl` (ndjson MIME types — checked **before** JSON because they contain "json"), `json` (**offloaded to a Web Worker** — see below), `binary` (image/audio/video/octet-stream), or `text`. Returns `{ kind, text?, parsed?, mediaType, wireBytes, decodedBytes?, rawText, bytes }`, where `rawText` is the un-pretty decoded text and `bytes` the decompressed bytes — both always present so the raw/hex view modes have a source for every kind.
- **JSON Web Worker** (`body/json-parse.worker.ts` + `body/json-parse-worker.ts`): `JSON.parse` and `JSON.stringify` (pretty-print) run in a dedicated module Worker so multi-MB bodies don't block the UI thread. `json-parse.ts` holds the pure `parseAndFormat()` function (imported by both the Worker script and Node-environment unit tests). `json-parse-worker.ts` is the main-thread client: a lazy singleton Worker with job-ID message routing that exposes `parseJson(text): Promise<ParseResult>`. The Worker is created on first JSON body and reused thereafter. A fatal worker error resets the singleton so the next parse creates a fresh one. `BodyPane` shows a skeleton while the Worker is in flight.
...
ui/
  src/
    ...
    body/           # Body decoding. decode.ts (chunks→bytes→decompress→classify json/jsonl/text/binary); json-parse.ts (pure parseAndFormat — shared by Worker and tests); json-parse.worker.ts (Web Worker script — JSON.parse + pretty-print off main thread); json-parse-worker.ts (main-thread Worker client — lazy singleton, parseJson()); sse.ts (parseSSEBlock, parseSSEBody, chunksToText); sse-stream.ts (SSEStreamState, feedChunk, applyRetention, chunkToText — incremental SSE parser)
    ...
    components/     # App-owned components, flat kebab-case: body-split, body-pane, live-indicator, stream-error-banner, json-viewer, plus v2.4 shell/chrome scaffolds (app-shell, top-bar, filter-bar, status-bar, command-palette, shortcuts-overlay, exchange-table, exchange-row, inspector, stream-view, event-log, method/status/connection atoms)
      ui/           # shadcn/ui primitives (Radix/cmdk wrappers): button, dialog, tabs, tooltip, dropdown-menu, command + EmptyState, MethodBadge, SimpleTooltip
