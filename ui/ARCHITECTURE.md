...
ui/
  src/
    ...
    body/           # Body decoding. decode.ts (chunks→bytes→decompress→classify json/ndjson/text/binary); json-parse-core.ts (pure parseAndFormat/parseWithTruncation/parseNdjson — shared by Worker and tests); json-parse.worker.ts (Web Worker script — parse + tree build + flatten off main thread, json + ndjson modes); json-parse.ts (main-thread Worker client — lazy singleton, parseJson()/parseNdjson()); sse.ts (parseSSEBlock, parseSSEBody, chunksToText); sse-stream.ts (SSEStreamState, feedChunk, applyRetention, chunkToText — incremental SSE parser)
