...
- `decodeBody` (`body/decode.ts`) is the pure async pipeline: concatenate chunks to bytes (`text` chunks UTF-8-encoded, `binary` chunks base64-decoded) → decompress via the browser `DecompressionStream` for `gzip`/`deflate`/`deflate-raw` (`br` is reported as not-decompressed, not attempted) → `TextDecoder` to text → classify into `kind`: `jsonl` (ndjson MIME types — checked **before** JSON because they contain "json"), `json` (pretty-printed), `binary` (image/audio/video/octet-stream), or `text`. Returns `{ kind, text?, mediaType, size }`.
