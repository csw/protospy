...
ui/
  src/
    ...
    state/          # store.ts (Zustand store + persist middleware + UI state + density-ownership data-density subscription + dev __test_store); reducer.ts (pure apply()); types.ts (Exchange/BodyState/ExchangeError shapes)
