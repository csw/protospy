...
- `decodeBody` (`body/decode.ts`) is the pure async pipeline: concatenate chunks to bytes (`text` chunks UTF-8-encoded, `binary` chunks base64-decoded) → decompress: `gzip`/`deflate`/`deflate-raw` via the browser `DecompressionStream`; `br` (brotli) via `brotli-dec-wasm` (a ~200 KB decode-only WASM module, lazy-loaded on first use via a module-level singleton) → `TextDecoder` to text → classify into `kind`: `jsonl` (ndjson MIME types — checked **before** JSON because they contain "json"), `json` (pretty-printed), `binary` (image/audio/video/octet-stream), or `text`. Returns `{ kind, text?, mediaType, size }`.
...
ui/
  ...
  playwright.config.ts # Playwright: ./browser, chromium, baseURL :5173, runs `pnpm dev` as webServer
