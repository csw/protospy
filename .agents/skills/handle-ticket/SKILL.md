...
## 2 — Enter a worktree and create the branch

This skill must run from a worktree. In Codex, start ticket work in a
Codex-native app worktree; do not assume a fixed worktree path such as
`.Codex/worktrees/`, and do not run `git worktree add` yourself. In Claude Code,
continue using the repository's existing `.claude/worktrees/<branch-name>`
workflow.

Inside the worktree, create or check out a recognizable ticket branch before
implementation:
