...
`AGENTS.md`: read the parent and inspect sibling titles, and open any
...
(`just codex-ticket $ARGUMENTS`, or `cs codex-ticket $ARGUMENTS` in the
...
In Codex, use `model: gpt-5.5` with high or xhigh reasoning for this diagnostic
pass.
...
**Spawn a `general-purpose` subagent for a lightweight visual check.** In Codex,
...
once after correcting any obvious spawn parameter problem: remove
Claude-only model names in Codex, omit `model` and `service_tier`, and fix
malformed agent type strings. If the typed reviewer still fails for a
