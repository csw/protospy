...
Codex model; never pass a Claude model name such as `haiku`.
Give it this prompt:
...
You also do **not** need to read the subproject's `AGENTS.md` manually — the
brief captured its rules, and the harness also auto-injects it the first time you
`Read` any file under that subproject's tree (e.g. `ui/AGENTS.md` on your first
