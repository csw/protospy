...
## 2 — Verify the worktree and create the branch
...
Inside the verified worktree, create or check out a recognizable ticket branch
before implementation:

- Prefer the `branchName` returned by Linear.
- If Linear does not return a usable branch name, use
  `codex/pro-NNN-short-title-slug`.
- Keep the ticket ID and a short title slug in the branch name even if you need
  to adjust the exact prefix or truncation.
...
When done, run the subproject's quality checks as listed in its `AGENTS.md`.
