...
The `obsidian` CLI connects to the running Obsidian app via a Unix domain socket at `~/.obsidian-cli.sock`. The sandbox blocks outbound Unix socket connections by default.

This is fixed in `~/.claude/settings.json` via `sandbox.network.allowUnixSockets: ["~/.obsidian-cli.sock"]` — no `dangerouslyDisableSandbox` override needed.
